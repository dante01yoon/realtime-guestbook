...
2. SQL Editor에서 `supabase/schema.sql` 내용을 실행해 `profiles`, `entries`, `comments` 테이블과 RLS 정책을 생성합니다.
3. Storage에 `guestbook` 버킷을 만들고 인증된 사용자만 업로드할 수 있도록 정책을 설정한 뒤(public read 권장) 클라이언트에서 해당 버킷을 사용합니다.
